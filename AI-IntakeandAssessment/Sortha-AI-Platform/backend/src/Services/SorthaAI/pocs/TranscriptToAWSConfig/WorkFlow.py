...
class TranscriptToAIF(WorkFlowBase):
    ...
    def extractResourceInformation(self, state: State):
        ...
        state.resources = structuredLLM.invoke(f'Provided following is the transcript generated after conversing with the customer. Analyse the conversation and predict the aws resouce configuration for the application: {state.transcript.content}').resources
        state.retry_count += 1
        return state
    ...
    def buildGraph(self):
        ...
        self.getStateGraph().add_conditional_edges(
            ...
            self.retryValidator,
            {
                "PASS": self.getEndNodePointer(),
                "RETRY": "extractResourceInformation"
            }
        )
        self.getStateGraph().add_edge("extractResourceInformation", self.getEndNodePointer())