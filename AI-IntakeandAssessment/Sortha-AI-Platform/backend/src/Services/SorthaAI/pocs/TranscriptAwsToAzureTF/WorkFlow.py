...
from .State import State, Resources, YesOrNo, TerraformResourceList, TerraFormCodeStructure
...
class TranscriptToAIF(WorkFlowBase):
    ...
    # def generateTerraformCode(self, state: State):
    #     structuredLLM = self.getLLM().with_structured_output(TerraformResourceList)
    #     state.terraform_resources = structuredLLM.invoke(f'Generate terraform code for the following azure resources. Also create any dependent resources : {state.azure_resources}').resources
    #     return state
    
    def generateTerraformCode(self, state: State):
        structuredLLM = self.getLLM().with_structured_output(TerraFormCodeStructure)
        state.terraform_code = structuredLLM.invoke(f'Generate a list of terraform code files for the following azure resources. Divide the code into smaller modules with the purpose of increasing the code reusability. Structure the code in multiple file to follow better coding standards. segregate the variables and module calling code. use azure verified modules as the northstar. create any dependent resources as well : {state.azure_resources}').content
        return state
    ...
    def buildGraph(self):
        ...
        self.getStateGraph().add_node("generateTerraformCode", self.generateTerraformCode)
        ...
        self.getStateGraph().add_edge("convertToAzureResources", "generateTerraformCode")
        self.getStateGraph().add_edge("generateTerraformCode", self.getEndNodePointer())