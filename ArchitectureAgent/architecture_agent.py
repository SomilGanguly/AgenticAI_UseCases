...
def architectureagent_conversation(orchestrator_id=None):
    load_dotenv()
    PROJECT_ENDPOINT = os.getenv("FOUNDRY_PROJECT_ENDPOINT")
    
    if not orchestrator_id:
        orchestrator_id = os.getenv("ARCHITECTURE_AGENT_OPENAPI_ID")
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        architectureagent_conversation(orchestrator_id=sys.argv[1])
    else:
        architectureagent_conversation()