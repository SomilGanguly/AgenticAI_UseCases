...
function redactSecrets(text, allowUnsafe) {
    if (allowUnsafe)
        return text;
    const patterns = [
...
function buildDevOpsBaseUrl(orgInput) {
    if (!orgInput)
        return '';
...
async function fetchTimeline(org, project, buildId, patOrToken, useBearer) {
    ...
    try {
        const resp = await axios_1.default.get(url, { headers: { Authorization: authHeader } });
        const records = resp.data?.records;
        if (!Array.isArray(records))
            throw new Error('Azure DevOps response did not contain an array of records.');
        return records;
    }
    catch (e) {
...
async function fetchFullLogText(logUrl, patOrToken, useBearer) {
    const authHeader = useBearer ? `Bearer ${patOrToken}` : 'Basic ' + Buffer.from(':' + patOrToken).toString('base64');
    try {
        const resp = await axios_1.default.get(logUrl, { headers: { Authorization: authHeader } });
        let lines = [];
...
async function analyzeFailures(org, project, buildId, patOrToken, useBearer) {
...
async function callOpenAI(endpoint, apiVersion, deployment, apiKey, content) {
    ...
    const resp = await axios_1.default.post(url, body, { headers });
...
async function run() {
    try {
        ...
        if (redacted !== logs)
            logDebug('Secrets redacted from logs.');
        const clipped = trimToMaxChars(redacted, isFinite(maxChars) ? maxChars : 40000);
        console.log(`clipped:${clipped}`);
        if (clipped.length !== redacted.length)
            logDebug(`Logs clipped to ${clipped.length} characters (maxChars=${maxChars}).`);
        ...
        if (outputFormat === 'json') {
            try {
                ...
                logDebug(`Parsed JSON severity=${severity}`);
            }
            catch {
                logDebug('JSON parse failed – treating output as plain text.');
            }
        }
        else if (output.toLowerCase().includes('severity: high')) {
            severity = 'high';
            logDebug('Heuristic detected high severity in output.');
        }
        if (outputFormat === 'markdown')
            tl.setVariable('LLMRCA.Markdown', output);
        else if (outputFormat === 'json')
            tl.setVariable('LLMRCA.Json', output);
        else
            tl.setVariable('LLMRCA.Text', output);
